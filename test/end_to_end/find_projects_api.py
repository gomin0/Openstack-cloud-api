...
async def find_projects_setup(async_db_session):
    ...
    user1 = User(openstack_id="user123", domain=domain, account_id="user1", name="사용자1", password="@!#32")
    user2 = User(openstack_id="user456", domain=domain, account_id="user2", name="사용자2", password="@!@3")
    project1 = Project(openstack_id="project123", domain=domain, name="프로젝트1")
    project2 = Project(openstack_id="project456", domain=domain, name="프로젝트2")
...
async def test_find_projects(async_client, async_db_session):
    # given
    await find_projects_setup(async_db_session)
...
async def test_find_projects_with_name_like(async_client, async_db_session):
    # given
    await find_projects_setup(async_db_session)
...
#
#
# def test_get_projects(test_client, db_session):
#     project_setup(db_session)
#
...
#
#
# def test_get_projects_with_name_like(test_client, db_session):
#     response = test_client.get("/projects?name_like=2")
#
#     assert response.status_code == 200
#     data = response.json()
#     assert len(data["projects"]) == 1
...
#
#
# def test_get_project(test_client, db_session):
