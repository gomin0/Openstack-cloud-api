...
async def login(
    ...
    user: User
    joined_projects: list[Project]
    user, joined_projects = await auth_service.authenticate_user_and_load_projects(
        session=session,
    ...
    if request.project_id is not None:
        # 요청 받은 프로젝트 조회 및 선택
        project: Project | None = next((proj for proj in joined_projects if proj.id == request.project_id), None)
        if project is None:
