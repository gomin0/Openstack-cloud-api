...
class SecurityGroupRuleResponse(BaseModel):
    ...
    protocol: str = Field(description="프로토콜", examples=["tcp"])
    direction: Literal["ingress", "egress"] = Field(description="방향", examples=["ingress"])
...
class SecurityGroupDetailResponses(BaseModel):
