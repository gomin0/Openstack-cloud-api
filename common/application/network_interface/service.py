...
class NetworkInterfaceService:
    ...
    async def attach_floating_ip_to_network_interface(
        ...
        network_interface: NetworkInterface = await self._get_network_interface_by_id(id_=network_interface_id)
        network_interface.validate_access_permission(project_id=project_id)

        floating_ip: FloatingIp = await self._get_floating_ip_by_id(floating_ip_id=floating_ip_id)
